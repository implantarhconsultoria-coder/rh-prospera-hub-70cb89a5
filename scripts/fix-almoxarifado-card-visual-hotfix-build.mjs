...
source = source
  .replaceAll('bg-gradient-to-r from-violet-50 to-white', 'bg-card')
  .replaceAll('border-violet-200', 'border-border')
  .replaceAll('bg-white', 'bg-card')
  .replaceAll('text-violet-700', 'text-primary')
  .replaceAll('text-slate-950', 'text-foreground')
  .replaceAll('text-slate-900', 'text-foreground')
  .replaceAll('text-slate-500', 'text-muted-foreground')
  .replaceAll('text-slate-400', 'text-muted-foreground')
  .replaceAll('bg-[#F7F8FC]', 'bg-background');

fs.writeFileSync(file, source, 'utf8');
console.log('[almox-card-hotfix] visual institucional escuro aplicado aos cards do Almoxarifado');
