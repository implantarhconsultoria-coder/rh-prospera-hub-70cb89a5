create or replace function public.almoxarifado_import_excel_batch_v4(p_lote uuid,p_seq integer,p_tipo text,p_company_code text,p_payload jsonb)
  ...
  if p_tipo='items' then
    ...
    with src as (select value v from jsonb_array_elements(p_payload)), mapped as (
      select s.v,i.id item_id from src s left join lateral(
        select ai.id from public.almoxarifado_itens ai where ai.importacao_lote_id=p_lote and (ai.codigo_topac=s.v->>'codigo' or ai.codigo_alternativo=s.v->>'codigo' or ai.codigo_sku=s.v->>'codigo')
        order by (lower(btrim(coalesce(ai.nome,'')))=lower(btrim(coalesce(s.v->>'descricao','')))) desc,ai.ativo desc,(ai.codigo_topac=s.v->>'codigo') desc,ai.planilha_linha nulls last,ai.id limit 1
      )i on true), ins as (
...
revoke all on function public.almoxarifado_import_excel_batch_v4(uuid,integer,text,text,jsonb) from public,anon,authenticated;
grant execute on function public.almoxarifado_import_excel_batch_v4(uuid,integer,text,text,jsonb) to service_role;
