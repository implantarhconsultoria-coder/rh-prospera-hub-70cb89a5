    ...
    FOREACH v_modulo IN ARRAY v_modulos LOOP
      ...
      ON CONFLICT (cpf_clean, modulo) DO UPDATE
         SET nome = EXCLUDED.nome,
             ...
             status = 'ativo', acesso_liberado = true, ativo = true, updated_at = now();
    END LOOP;
  END IF;

  RETURN jsonb_build_object('ok', true, 'funcionario_id', p_funcionario_id, 'ativo', p_ativo, 'modulos', to_jsonb(v_modulos));
END;
$$;
...
AS $$
BEGIN
  IF NEW.ativo IS FALSE OR lower(COALESCE(NEW.status, '')) IN ('desligado', 'excluido', 'inativo') THEN
    UPDATE public.acessos_externos
       SET status = 'bloqueado', acesso_liberado = false, updated_at = now()
...
FOR EACH ROW EXECUTE FUNCTION public.topac_bloquear_acessos_funcionario_inativo();
