...
CREATE INDEX IF NOT EXISTS idx_ativos_chassi ON public.ativos(chassi);
...
DROP POLICY IF EXISTS topac_frota_ativos_select ON public.ativos;
CREATE POLICY topac_frota_ativos_select ON public.ativos
...
DROP POLICY IF EXISTS topac_frota_ativos_insert ON public.ativos;
CREATE POLICY topac_frota_ativos_insert ON public.ativos
  FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid()
        AND ur.role::text = ANY (ARRAY['admin','operacional','filial_matriz','filial_praia','filial_goiania'])
    )
  );

DROP POLICY IF EXISTS topac_frota_ativos_update ON public.ativos;
CREATE POLICY topac_frota_ativos_update ON public.ativos
...
DROP POLICY IF EXISTS topac_frota_ativos_delete ON public.ativos;
CREATE POLICY topac_frota_ativos_delete ON public.ativos
...
DROP POLICY IF EXISTS topac_frota_manutencoes_all ON public.veiculo_manutencoes;
CREATE POLICY topac_frota_manutencoes_all ON public.veiculo_manutencoes
...
DROP POLICY IF EXISTS topac_frota_storage_select ON storage.objects;
CREATE POLICY topac_frota_storage_select ON storage.objects
  FOR SELECT TO authenticated
  USING (bucket_id = 'documentos-ativos');

DROP POLICY IF EXISTS topac_frota_storage_insert ON storage.objects;
CREATE POLICY topac_frota_storage_insert ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'documentos-ativos');

DROP POLICY IF EXISTS topac_frota_storage_update ON storage.objects;
CREATE POLICY topac_frota_storage_update ON storage.objects
  FOR UPDATE TO authenticated
  USING (bucket_id = 'documentos-ativos')
  WITH CHECK (bucket_id = 'documentos-ativos');

DROP POLICY IF EXISTS topac_frota_storage_delete ON storage.objects;
CREATE POLICY topac_frota_storage_delete ON storage.objects
  FOR DELETE TO authenticated
  USING (bucket_id = 'documentos-ativos');
