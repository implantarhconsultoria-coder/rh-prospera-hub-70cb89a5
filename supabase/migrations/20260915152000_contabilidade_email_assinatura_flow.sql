-- Contabilidade -> Assinatura Digital
-- Adiciona rastreio de e-mails do ciclo e inclui adiantamentos na visão administrativa.

alter table public.contabilidade_folha_ciclos
  add column if not exists email_envio_status text,
  add column if not exists email_envio_em timestamptz,
  add column if not exists email_retorno_status text,
  add column if not exists email_retorno_em timestamptz;

create or replace view public.payroll_admin_status_v as
select
  d.id as document_id,
  d.company_id,
...
where d.is_current = true
  and d.document_type in ('HOLERITE','ADIANTAMENTO');
