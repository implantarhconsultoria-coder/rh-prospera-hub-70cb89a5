  ...
  IF v_client_id IS NOT NULL THEN
    SELECT rp.id
     ...
     WHERE rp.user_id = v_user_id
       AND strpos(COALESCE(rp.dispositivo, ''), '"client_id":"' || v_client_id || '"') > 0
...
DECLARE
  ...
  v_user uuid;
  v_count int;
  v_data_brasilia date := timezone('America/Sao_Paulo', now())::date;
BEGIN
  BEGIN
    v := public._app_mecanico_get_acesso(p_acesso_id);
  EXCEPTION WHEN OTHERS THEN
  ...
  SELECT count(*)
   ...
   WHERE user_id = v_user
     AND data = v_data_brasilia;

  RETURN jsonb_build_object(
    'ok', true,
    'batidas_hoje', v_count,
    'data_brasilia', v_data_brasilia
  );
END;
$function$;
