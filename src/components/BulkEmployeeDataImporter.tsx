...
import { extractPdfTextByLines } from '@/lib/pdf';
...
type ImportRow = {
  ...
  matchStatus: 'matched' | 'unmatched' | 'ambiguous' | 'company-error';
  message?: string;
};
...
const extractCompanyName = (text: string) => {
  const match = text.match(/Raz[aã]o\s+Social\s*:\s*(.+?)(?=\s+(?:CNPJ\s*\/\s*CEI|Endere[cç]o\s*:|Bairro\s*:|Cidade\s*:|Nome\s*:)|$)/i);
...
const actionForCpf = (current: string, source: string): ImportRow['cpfAction'] => {
  if (!source) return 'missing-source';
  if (!current) return 'fill';
  return digits(current) === digits(source) ? 'same' : 'conflict';
};

const actionForBirth = (current: string, source: string): ImportRow['birthAction'] => {
  if (!source) return 'missing-source';
  if (!current) return 'fill';
  return normalizeDate(current) === normalizeDate(source) ? 'same' : 'conflict';
};

const actionLabel = (action: ImportRow['cpfAction'] | ImportRow['birthAction']) => {
  if (action === 'fill') return 'PREENCHER';
  if (action === 'same') return 'JÁ CONFERE';
  if (action === 'conflict') return 'CONFERIR';
  return 'NÃO ENCONTRADO';
};

const actionClass = (action: ImportRow['cpfAction'] | ImportRow['birthAction']) => {
  if (action === 'fill') return 'bg-primary/15 text-primary border-primary/30';
  if (action === 'same') return 'bg-success/15 text-success border-success/30';
  if (action === 'conflict') return 'bg-destructive/15 text-destructive border-destructive/30';
...
const BulkEmployeeDataImporter: React.FC<Props> = ({ open, onOpenChange, employees, companies, companyId, onSaved }) => {
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      try {
        ...
        if (!text.trim()) {
          nextIssues.push({ fileName: file.name, message: 'PDF sem camada de texto legível. Este arquivo precisa ser revisado antes da atualização automática.' });
          continue;
        }
        ...
        const parsedPeople = parseEmployeesReport(text);
        if (!parsedPeople.length) {
          nextIssues.push({ fileName: file.name, message: 'Nenhum funcionário foi identificado no padrão Nome / Data de Nascimento / CPF.' });
          continue;
        }
        ...
        for (const parsed of parsedPeople) {
          ...
          nextRows.push({
            ...
            birthAction: employee ? actionForBirth(employee.dataNascimento, parsed.birthDate) : 'missing-source',
            matchStatus,
  ...
  const saveMissingFields = async () => {
    ...
    for (const row of pendingRows) {
      ...
      if (row.cpfAction === 'fill' && row.parsed.cpf) {
    ...
    <DialogContent className="max-h-[92vh] max-w-6xl overflow-y-auto">
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Envie um ou vários PDFs. A plataforma identifica empresa, funcionário, CPF e data de nascimento, compara com o cadastro atual e preenche somente campos vazios. Dados já existentes nunca são sobrescritos automaticamente.</p>
          {selectedCompany && <p className="mt-2 text-xs font-medium">Empresa selecionada: {selectedCompany.name} — {selectedCompany.cnpj}</p>}
        </div>

        <input ref={inputRef} type="file" accept="application/pdf,.pdf" multiple className="hidden" onChange={(event) => { const files = Array.from(event.target.files || []); void processFiles(files); }} />
        ...
        {processing && <div className="flex items-center gap-2 rounded-lg border p-4 text-sm"><Loader2 className="h-4 w-4 animate-spin" /> Processando os relatórios em massa...</div>}
        ...
        {issues.length > 0 && <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4"><p className="mb-2 text-sm font-semibold text-destructive">Arquivos com pendência</p>{issues.map((issue, index) => <p key={`${issue.fileName}-${index}`} className="text-xs"><b>{issue.fileName}:</b> {issue.message}</p>)}</div>}
      </div>

      <DialogFooter className="gap-2 sm:gap-0">
        <Button variant="outline" onClick={() => onOpenChange(false)} disabled={saving}>Fechar</Button>
        <Button onClick={() => void saveMissingFields()} disabled={!pendingRows.length || processing || saving}>{saving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Atualizando...</> : <>Atualizar {pendingRows.length} cadastro(s)</>}</Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>;
};
