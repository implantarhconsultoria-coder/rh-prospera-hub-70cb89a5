  ...
  const supportCards = [
    {
      ...
      icon: FileUp,
      title: 'PDF assinado',
      subtitle: 'Arquivar o relatório já assinado',
    },
    {
      id: 'autorizacao' as const,
      icon: AlertTriangle,
      title: 'Retirada fora do horário',
      subtitle: 'Registrar autorização excepcional',
    },
    {
      id: 'mensal' as const,
      icon: CalendarDays,
      title: 'Fechamento mensal',
      subtitle: 'Congelar entradas, retiradas e saldo',
    },
  ];
      ...
      <div className="almox-support-grid">
        {supportCards.map(({ id, icon: Icon, title, subtitle }) => (
          <button
            key={id}
            type="button"
            className={`almox-support-card ${supportView === id ? 'active' : ''}`}
            onClick={() => setSupportView((current) => current === id ? null : id)}
          >
        ...
        <article className="almox-support-detail">
          <div className="almox-support-detail-head">
            <div className="almox-section-title">
              <FileUp size={19} />
              <div><strong>Arquivar PDF assinado</strong><small>Digitalize o relatório assinado manualmente e guarde no histórico.</small></div>
            </div>
          ...
          <div className="almox-form-grid">
            ...
            <label className="almox-field">
              ...
              <input type="date" value={dataReferencia} min="2026-09-16" onChange={(e) => setDataReferencia(e.target.value)} />
            </label>
            <label className="almox-field full">
              <span>PDF assinado</span>
              <input type="file" accept="application/pdf" onChange={(e) => setArquivo(e.target.files?.[0] || null)} />
            </label>
            <div className="almox-form-actions">
              <button type="button" className="almox-primary" onClick={arquivarPdf} disabled={enviando}>
                {enviando ? <Loader2 className="spin" size={17} /> : <FileUp size={17} />}
                {enviando ? 'Arquivando...' : 'Arquivar PDF'}
              </button>
            </div>
          </div>
        </article>
        ...
        <article className="almox-support-detail">
          <div className="almox-support-detail-head">
            <div className="almox-section-title">
              ...
              <div><strong>Retirada fora do horário</strong><small>Registre a autorização antes de realizar a retirada excepcional.</small></div>
            </div>
            <button type="button" className="almox-support-close" onClick={() => setSupportView(null)} aria-label="Fechar"><X size={17} /></button>
          </div>
          <div className="almox-form-grid">
            <label className="almox-field">
              <span>Funcionário que receberá o material</span>
              <select value={funcionarioId} onChange={(e) => setFuncionarioId(e.target.value)}>
                <option value="">Selecionar funcionário...</option>
                {funcionarios.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}
              </select>
            ...
            <label className="almox-field">
              ...
              <input value={autorizadoPor} onChange={(e) => setAutorizadoPor(e.target.value)} placeholder="Nome do responsável" />
            </label>
            <label className="almox-field full">
              <span>Motivo da autorização</span>
              <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} placeholder="Informe o motivo da retirada excepcional" rows={3} />
            </label>
            <div className="almox-form-actions">
              <button type="button" className="almox-primary" onClick={registrarAutorizacao} disabled={salvandoAutorizacao}>
                {salvandoAutorizacao ? <Loader2 className="spin" size={17} /> : <AlertTriangle size={17} />}
                {salvandoAutorizacao ? 'Registrando...' : 'Registrar autorização'}
              </button>
            </div>
          </div>
        </article>
        ...
        <article className="almox-support-detail">
          <div className="almox-support-detail-head">
            <div className="almox-section-title">
              ...
              <div><strong>Fechamento mensal</strong><small>Persiste entradas, retiradas, saldo e detalhe por funcionário no histórico.</small></div>
            </div>
            <button type="button" className="almox-support-close" onClick={() => setSupportView(null)} aria-label="Fechar"><X size={17} /></button>
          </div>
          <div className="almox-form-grid">
            <label className="almox-field">
              ...
              <input type="month" min="2026-09" value={competencia} onChange={(e) => setCompetencia(e.target.value)} />
            </label>
            <div className="almox-form-actions">
              <button type="button" className="almox-primary" onClick={fecharMes} disabled={fechandoMes}>
                {fechandoMes ? <Loader2 className="spin" size={17} /> : <CalendarDays size={17} />}
                {fechandoMes ? 'Fechando...' : 'Fechar e persistir mês'}
              </button>
            </div>
          </div>
        </article>
