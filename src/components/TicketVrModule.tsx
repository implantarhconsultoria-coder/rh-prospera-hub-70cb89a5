...
const TicketVrModule: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.success('Dados do Ticket puxados e cálculo preparado para conferência.');
    } catch (error: any) {
  ...
  const approve = async () => {
    ...
    try {
      ...
      toast.success(`Conferência aprovada — versão ${data.versao}. O TXT ainda NÃO foi gerado.`);
  ...
  const generateTxt = async () => {
    ...
    try {
      ...
      toast.success('TXT Ticket gerado somente após sua autorização final.');
      await loadHistoryOnly();
    } catch (error: any) {
  ...
  return <div className="card-premium p-5 space-y-4 border-l-4 border-primary">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      <div>
        ...
        <p className="text-xs text-muted-foreground">O módulo não consulta funcionários, faltas ou VR automaticamente. Goiânia permanece fora deste módulo.</p>
      </div>
