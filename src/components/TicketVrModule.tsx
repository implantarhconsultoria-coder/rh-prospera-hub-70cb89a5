...
import { AlertTriangle, CheckCircle2, FileDown, RefreshCw, Save, Settings2, UtensilsCrossed } from 'lucide-react';
...
const TicketVrModule: React.FC = () => {
  ...
  const ticketCompanies = useMemo(() => companies.filter((company) => !`${company.name} ${company.codigo || ''}`.toLowerCase().includes('goian') && !String(company.codigo || '').toLowerCase().includes('gyn')), [companies]);
  ...
  useEffect(() => { if (!companyId && ticketCompanies.length) setCompanyId(ticketCompanies[0].id); }, [companyId, ticketCompanies]);

  const load = async () => {
    if (!companyId || !competencia) return;
    setLoading(true); setApprovedId(''); setApprovedVersion(0);
    try {
      const [{ data: companyData, error: companyError }, { data: employeeData, error: employeeError }, { data: launchData, error: launchError }, { data: configData }, { data: historyData }] = await Promise.all([
        (supabase as any).from('empresas').select('id,nome,razao_social,cnpj,codigo,status').eq('id', companyId).single(),
        (supabase as any).from('funcionarios').select('id,nome,cpf,data_nascimento,data_admissao,data_demissao,vr_ativo,vr_diario,status,ativo,company_id,empresa_id').or(`company_id.eq.${companyId},empresa_id.eq.${companyId}`).eq('vr_ativo', true),
      ...
      if (companyError) throw companyError; if (employeeError) throw employeeError; if (launchError) throw launchError;
      setCompany(companyData); setConfig(configData ? configFromRow(configData) : emptyConfig()); setHistory(historyData || []);
      ...
      const calculated = (employeeData || []).filter((employee: any) => {
        const admission = String(employee.data_admissao || ''); const dismissal = String(employee.data_demissao || '');
        ...
        if (paidDays < 0) pending.push('QUANTIDADE DE DIAS INVÁLIDA');
      ...
      setRows(calculated);
    } catch (error: any) { toast.error(`Não foi possível calcular o Ticket VR: ${error?.message || 'erro inesperado'}`); }
    finally { setLoading(false); }
  };

  useEffect(() => { void load(); }, [companyId, competencia]);

  const preflight = useMemo(() => company ? ticketVrPreflight({ cnpj: company.cnpj, nome: company.nome }, config, rows) : ['Empresa não carregada.'], [company, config, rows]);
  const total = rows.reduce((sum, row) => sum + row.total, 0);
  const totalDays = rows.reduce((sum, row) => sum + row.diasPagos, 0);

  const saveConfig = async () => {
    if (!companyId) return; setSavingConfig(true);
    try {
      ...
      if (error) throw error; toast.success('Configuração Ticket salva.'); setConfigOpen(false); await load();
    } catch (error: any) { toast.error(`Erro ao salvar configuração Ticket: ${error?.message || 'falha'}`); }
    finally { setSavingConfig(false); }
  };

  const printReport = () => {
    if (!company) return;
    const win = window.open('', '_blank'); if (!win) return toast.error('Libere pop-ups para gerar o PDF.');
  ...
  const approve = async () => {
    ...
    try {
      ...
      const { data: auth } = await supabase.auth.getUser();
      const snapshot = { company, competencia, config, rows, total, totalDays, preflight, calculadoEm: new Date().toISOString(), regra: 'VR diário do cadastro × dias úteis individuais − faltas com data; HE não gera VR' };
      ...
      if (error) throw error; setApprovedId(data.id); setApprovedVersion(data.versao); toast.success(`Conferência aprovada — versão ${data.versao}.`); await loadHistoryOnly();
    } catch (error: any) { toast.error(`Não foi possível aprovar: ${error?.message || 'falha'}`); }
  };

  const loadHistoryOnly = async () => {
    const { data } = await (supabase as any).from('ticket_vr_geracoes').select('id,competencia,versao,status,total_funcionarios,total_dias_vr,valor_total,nome_arquivo,checksum_sha256,created_at').eq('company_id', companyId).eq('competencia', competencia).order('versao', { ascending: false }).limit(20); setHistory(data || []);
  };

  const generateTxt = async () => {
    if (!approvedId || !company) return; if (preflight.length) return toast.error('O cálculo mudou e voltou a ter pendências. Atualize e aprove novamente.');
    try {
      const txt = buildTicketVrTxt({ nome: company.razao_social || company.nome, cnpj: company.cnpj }, competencia, config, rows);
      const checksum = await sha256(txt); const safeCompany = String(company.nome || 'EMPRESA').normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^A-Za-z0-9]+/g, '_').replace(/^_|_$/g, ''); const filename = `TICKET_VR_${safeCompany}_${competencia.replace('-', '')}_V${approvedVersion}.TXT`;
      const { error } = await (supabase as any).from('ticket_vr_geracoes').update({ status: 'TXT GERADO', nome_arquivo: filename, checksum_sha256: checksum, txt_conteudo: txt, txt_gerado_em: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', approvedId); if (error) throw error;
      downloadTicketTxt(txt, filename); toast.success('TXT Ticket gerado no layout posicional de 164 caracteres.'); await loadHistoryOnly();
    } catch (error: any) { toast.error(`TXT não gerado: ${error?.message || 'falha estrutural'}`); }
  };

  return <div className="card-premium p-5 space-y-4 border-l-4 border-primary">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs uppercase text-muted-foreground flex items-center gap-2"><UtensilsCrossed className="w-4 h-4"/>Fechamento / Benefícios / VR</p><h2 className="text-lg font-bold">TICKET — GERAR ARQUIVO VR</h2><p className="text-xs text-muted-foreground">Conferência obrigatória antes do TXT. Goiânia permanece fora deste módulo.</p></div><Button variant="outline" onClick={() => setConfigOpen((value) => !value)}><Settings2 className="w-4 h-4 mr-2"/>Configuração Ticket</Button></div>
    <div className="flex flex-wrap gap-3"><select value={companyId} onChange={(event) => setCompanyId(event.target.value)} className="border rounded-lg bg-background px-3 py-2 text-sm min-w-64">{ticketCompanies.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><Input type="month" value={competencia} onChange={(event) => setCompetencia(event.target.value)} className="w-48"/><Button variant="outline" disabled={loading} onClick={() => void load()}><RefreshCw className={`w-4 h-4 mr-2 ${loading ? 'animate-spin' : ''}`}/>Atualizar cálculo</Button></div>
    {configOpen && <div className="rounded-xl border p-4 space-y-3 bg-muted/20"><p className="text-sm font-bold">Configuração estrutural Ticket — por CNPJ</p><p className="text-xs text-muted-foreground">Não altera valor de VR. Estes campos existem somente para preencher as posições obrigatórias do arquivo oficial Ticket.</p><div className="grid md:grid-cols-3 gap-2"><Cfg label="Código Cliente Ticket (10 posições)" value={config.codigoClienteTicket} set={(v) => setConfig({...config,codigoClienteTicket:v})}/><Cfg label="Unidade de entrega" value={config.unidadeEntrega} set={(v) => setConfig({...config,unidadeEntrega:v})}/><Cfg label="Departamento" value={config.departamento} set={(v) => setConfig({...config,departamento:v})}/><Cfg label="Logradouro" value={config.logradouro} set={(v) => setConfig({...config,logradouro:v})}/><Cfg label="Número" value={config.numero} set={(v) => setConfig({...config,numero:v})}/><Cfg label="Cidade" value={config.cidade} set={(v) => setConfig({...config,cidade:v})}/><Cfg label="Bairro" value={config.bairro} set={(v) => setConfig({...config,bairro:v})}/><Cfg label="CEP" value={config.cep} set={(v) => setConfig({...config,cep:v})}/><Cfg label="UF" value={config.uf} set={(v) => setConfig({...config,uf:v})}/><Cfg label="Interlocutor" value={config.interlocutor} set={(v) => setConfig({...config,interlocutor:v})}/><Cfg label="DDD" value={config.ddd} set={(v) => setConfig({...config,ddd:v})}/><Cfg label="Telefone" value={config.telefone} set={(v) => setConfig({...config,telefone:v})}/></div><Button disabled={savingConfig} onClick={() => void saveConfig()}><Save className="w-4 h-4 mr-2"/>{savingConfig ? 'Salvando...' : 'Salvar configuração'}</Button></div>}
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3"><Kpi label="Funcionários" value={String(rows.length)}/><Kpi label="Dias VR" value={String(totalDays)}/><Kpi label="Total empresa" value={formatCurrency(total)}/><Kpi label="Preflight" value={preflight.length ? `${preflight.length} pendência(s)` : 'VALIDADO'} danger={preflight.length > 0}/></div>
    ...
    <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={printReport}><FileDown className="w-4 h-4 mr-2"/>Gerar relatório PDF</Button><Button disabled={preflight.length > 0 || loading} onClick={() => void approve()}><CheckCircle2 className="w-4 h-4 mr-2"/>Aprovar conferência</Button><Button disabled={!approvedId || preflight.length > 0} onClick={() => void generateTxt()}><FileDown className="w-4 h-4 mr-2"/>Gerar TXT Ticket</Button></div>
