...
import { AlertTriangle, CheckCircle2, Clock3, Eye, FileCheck2, Loader2, MailCheck, RefreshCw, Send, WalletCards } from 'lucide-react';
import { toast } from 'sonner';
import { supabase } from '@/integrations/supabase/client';
import { Button } from '@/components/ui/button';

type Company = { id:string; nome:string; cnpj?:string; codigo?:string };
type Cycle = { id:string; empresa_id:string; competencia:string; tipo:'adiantamento'|'pagamento'; status:string; apontamento_liberado_em?:string|null; contabilidade_recebeu_em?:string|null; enviado_em?:string|null; conferido_em?:string|null; observacao?:string|null; email_envio_status?:string|null; email_retorno_status?:string|null };
...
type Upload = { id:string; ciclo_id:string; empresa_id:string; arquivo_nome:string; storage_bucket:string; storage_path:string; created_at:string; formalizacao_email_status?:string|null };
  ...
  const approve = async (cycle:Cycle) => {
    ...
    try {
      ...
      if (result.email_status === 'enviado') {
        toast.success(`Conferido / OK. ${released} recibo(s) liberado(s) na Assinatura Digital e e-mail de resposta enviado à Contabilidade.`);
      } else {
        toast.warning(`Conferido / OK. ${released} recibo(s) foram liberados, mas o e-mail de resposta não saiu. ${result.email_error || ''}`.trim());
      }
      await load(true);
  ...
  return createPortal(
    <section className="mb-5 overflow-hidden rounded-xl border border-[#352741] bg-[#05070b] shadow-[0_18px_50px_rgba(0,0,0,.22)]">
      <div className="flex flex-col gap-3 border-b border-[#28212f] bg-[radial-gradient(circle_at_10%_0%,rgba(139,34,255,.15),transparent_38%)] px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
        <div><div className="text-[10px] font-black uppercase tracking-[.18em] text-[#a855f7]">Fluxo mensal · {monthLabel(state?.competence||'')}</div><h2 className="mt-1 text-lg font-black text-white">Adiantamento e Pagamento</h2><p className="mt-1 text-xs text-zinc-500">A Contabilidade envia os PDFs e o sistema já prepara a Assinatura Digital. Você confere aqui; só o seu OK libera o documento para o funcionário.</p></div>
      ...
      {loading && !state ? <div className="flex h-28 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-violet-400" /></div> : <div className="grid gap-3 p-4 xl:grid-cols-2">
        {cycleRows.map(cycle => {
          ...
          return <div key={cycle.id} className="rounded-xl border border-[#2a2630] bg-[#080a0e] p-4">
            ...
            {cycle.email_retorno_status==='enviado' && <div className="mt-2 inline-flex items-center gap-2 rounded-md border border-emerald-500/20 bg-emerald-500/[.04] px-2.5 py-1.5 text-[10px] font-bold text-emerald-200"><MailCheck className="h-3.5 w-3.5"/>E-mail de OK enviado à Contabilidade</div>}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {cycle.status==='aguardando_conferencia' && <><Button size="sm" onClick={()=>void approve(cycle)} disabled={busy===cycle.id} className="bg-emerald-600 text-white hover:bg-emerald-500">{busy===cycle.id?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<CheckCircle2 className="mr-2 h-4 w-4"/>}Conferir, autorizar e responder</Button><Button size="sm" variant="outline" onClick={()=>{setIssueCycle(cycle.id);setIssueText('');}} className="border-rose-500/30 bg-rose-500/[.04] text-rose-200"><AlertTriangle className="mr-2 h-4 w-4"/>Informar pendência</Button></>}
              {cycle.status==='conferido' && <span className="inline-flex items-center gap-2 rounded-md border border-emerald-500/20 bg-emerald-500/[.04] px-3 py-2 text-[11px] font-bold text-emerald-300"><CheckCircle2 className="h-4 w-4"/>OK finalizado · funcionário liberado</span>}
