...
export const ticketVrPreflight = (company: { cnpj?: string; nome?: string }, config: TicketVrConfig, rows: TicketVrRow[]) => {
  ...
  if (ASCII(config.codigoClienteTicket).length !== 10) errors.push('Código Cliente Ticket deve possuir exatamente 10 posições conforme o modelo.');
...
export const buildTicketVrTxt = (company: { nome: string; cnpj: string }, competencia: string, config: TicketVrConfig, rows: TicketVrRow[]) => {
  ...
  const h = blankLine();
  put(h, 1, 5, 'LSUP0'); put(h, 6, 3, 'SUP'); put(h, 25, 8, compactDate()); put(h, 33, 8, compactTime()); put(h, 41, 18, 'LAYOUT-16/06/2014');
  output.push(h.join(''));

  const [year, month] = competencia.split('-').map(Number);
  const competenceStart = `${year}-${String(month).padStart(2, '0')}-01`;
  const competenceEnd = `${year}-${String(month).padStart(2, '0')}-${String(new Date(year, month, 0).getDate()).padStart(2, '0')}`;
  const r20 = blankLine();
  put(r20, 1, 5, 'TR020'); put(r20, 6, 10, pad(config.codigoClienteTicket, 10)); put(r20, 16, 30, pad(company.nome, 30)); put(r20, 46, 8, compactDate()); put(r20, 54, 8, date8(competenceEnd)); put(r20, 62, 1, 'C'); put(r20, 80, 2, '03'); put(r20, 101, 4, '0434'); put(r20, 153, 3, 'SUP'); put(r20, 159, 6, sequence(seq++));
  output.push(r20.join(''));

  const r22 = blankLine();
  put(r22, 1, 5, 'TR022'); put(r22, 6, 25, pad(config.unidadeEntrega, 25)); put(r22, 32, 1, pad(config.tipoLogradouro || 'R', 1)); put(r22, 36, 30, pad(config.logradouro, 30)); put(r22, 66, 14, pad(config.numero, 14)); put(r22, 82, 25, pad(config.cidade, 25)); put(r22, 107, 15, pad(config.bairro, 15)); put(r22, 122, 5, digits(config.cep).slice(0, 5).padEnd(5, ' ')); put(r22, 127, 2, pad(config.uf, 2)); put(r22, 129, 20, pad(config.interlocutor, 20)); put(r22, 149, 3, digits(config.ddd).padStart(3, '0').slice(-3)); put(r22, 152, 7, digits(config.telefone).slice(-7).padStart(7, ' ')); put(r22, 159, 6, sequence(seq++));
  output.push(r22.join(''));

  rows.forEach((row) => {
    const cpf = digits(row.cpf);
    const detail = blankLine();
    put(detail, 1, 5, 'TR023'); put(detail, 6, 25, pad(config.departamento || 'DEPARTAMENTO', 25)); put(detail, 32, 11, cpf); put(detail, 43, 8, date8(row.dataNascimento)); put(detail, 70, 25, pad(config.unidadeEntrega, 25)); put(detail, 96, 3, '001'); put(detail, 99, 3, '010'); put(detail, 102, 8, money8(row.total)); put(detail, 110, 1, 'R'); put(detail, 111, 35, pad(row.nome, 35)); put(detail, 148, 11, cpf); put(detail, 159, 6, sequence(seq++));
    output.push(detail.join(''));
  });

  const total = rows.reduce((sum, row) => sum + row.total, 0);
  const trailer = blankLine();
  put(trailer, 1, 5, 'TR029'); put(trailer, 6, 8, String(rows.length).padStart(8, '0')); put(trailer, 14, 14, money14(total)); put(trailer, 159, 6, sequence(seq++));
  output.push(trailer.join(''));

  const fileTrailer = blankLine();
  put(fileTrailer, 1, 5, 'LSUP9'); put(fileTrailer, 6, 8, '00000001'); put(fileTrailer, 14, 8, String(rows.length).padStart(8, '0')); put(fileTrailer, 22, 8, String(rows.length).padStart(8, '0'));
