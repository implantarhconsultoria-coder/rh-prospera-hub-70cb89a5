...
import {
  ...
  MessageCircle, ReceiptText, RefreshCw, Share2, ShieldCheck, XCircle,
} from "lucide-react";
...
type Step = "solicitar" | "aguardando" | "liberado" | "bomba" | "painel" | "recibo" | "revisao" | "negado" | "ok";
...
const stepFromStatus = (status?: string): Step =>
  status === "autorizado" ? "liberado" : status === "negado" ? "negado" : "aguardando";
...
const fuelErrorMessage = (code?: string) => {
  ...
  if (code === "foto_recibo_obrigatoria") return "A foto do recibo do posto é obrigatória.";
...
export default function AbastecimentoPageV4() {
  ...
  const [camRecibo, setCamRecibo] = useState(false);
  const [fotoBombaUrl, setFotoBombaUrl] = useState("");
  const [fotoPainelUrl, setFotoPainelUrl] = useState("");
  const [fotoReciboUrl, setFotoReciboUrl] = useState("");
  ...
  const chooseAuthorizedStep = useCallback((bomba = fotoBombaUrl, painel = fotoPainelUrl, recibo = fotoReciboUrl): Step => {
    if (bomba && painel && recibo) return "revisao";
    if (bomba && painel) return "recibo";
    if (bomba) return "painel";
    return "liberado";
  }, [fotoBombaUrl, fotoPainelUrl, fotoReciboUrl]);
  ...
  const restaurarFotos = useCallback(async (authorizationId: string) => {
    ...
    const result = data as {
      ok?: boolean;
      bomba_path?: string | null;
      painel_path?: string | null;
      recibo_path?: string | null;
    } | null;
    if (error || !result?.ok) return { bomba: "", painel: "", recibo: "" };

    const bomba = publicStorageUrl(result.bomba_path);
    const painel = publicStorageUrl(result.painel_path);
    const recibo = publicStorageUrl(result.recibo_path);
    if (bomba) setFotoBombaUrl(bomba);
    if (painel) setFotoPainelUrl(painel);
    if (recibo) setFotoReciboUrl(recibo);
    return { bomba, painel, recibo };
  }, [mecanico.acesso_id]);
  ...
  const carregarContexto = useCallback(async () => {
    ...
    try {
      ...
      const vehicleList = (result.veiculos || [])
        .map(v => ({ ...v, placa: normalizePlate(v.placa) }))
        .filter(v => v.placa);
      ...
      else if (active.status === "autorizado") setStep(
        saved.bomba && saved.painel && saved.recibo ? "revisao" :
        saved.bomba && saved.painel ? "recibo" :
        saved.bomba ? "painel" : "liberado"
      );
  ...
  const atualizarStatus = useCallback(async (silent = false) => {
    ...
    else if (current.status === "autorizado") setStep(chooseAuthorizedStep());
    return current;
  }, [autorizacao?.id, mecanico.acesso_id, chooseAuthorizedStep]);
  ...
  const onCaptureRecibo = async (blob: Blob) => {
    if (!autorizacao || autorizacao.status !== "autorizado") throw new Error("Aguarde a liberação antes de continuar.");
    if (!fotoBombaUrl || !fotoPainelUrl) throw new Error("Tire primeiro as fotos da bomba e do painel.");
    setLoading(true);
    try {
      const url = await uploadFoto("abastecimento-fotos", mecanico.acesso_id, `recibo-posto-${autorizacao.id}`, blob);
      setFotoReciboUrl(url);
      toast.success("Foto do recibo salva.");
      setStep("revisao");
    } finally {
      setLoading(false);
    }
  ...
  const finalizar = async () => {
    ...
    if (!fotoBombaUrl || !fotoPainelUrl || !fotoReciboUrl) return toast.error("As três fotos são obrigatórias.");
    ...
    try {
      ...
      const info: ReceiptInfo = {
        ...
        postoEndereco: station?.endereco || "",
  ...
  return <div className="space-y-4">
    <Card className="p-4">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-base font-bold">Solicitação de Abastecimento</h1><p className="text-xs text-muted-foreground">Solicite, aguarde a liberação e depois tire 3 fotos.</p></div>
      </div>
    </Card>
    ...
    {step === "aguardando" && autorizacao && <Card className="space-y-4 p-5">
      <div className="flex items-center gap-3 text-amber-500"><Clock3 className="h-7 w-7" /><div><h2 className="font-bold">Aguardando liberação</h2><p className="text-xs text-muted-foreground">A solicitação está no painel da administração.</p></div></div>
      <RequestSummary auth={autorizacao} posto={postoAtual} />
      <div className="grid gap-2 sm:grid-cols-2">{WHATSAPP_RECIPIENTS.map(r => <Button key={r.phone} onClick={() => abrirWhatsApp(r.phone)}><MessageCircle className="mr-2 h-4 w-4" />WhatsApp {r.label}</Button>)}</div>
    ...
    {step === "liberado" && autorizacao && <Card className="space-y-4 border-emerald-500/30 p-5">
      <div className="flex items-center gap-3 text-emerald-500"><ShieldCheck className="h-8 w-8" /><div><h2 className="text-lg font-bold">ABASTECIMENTO LIBERADO</h2><p className="text-xs text-muted-foreground">Agora tire as 3 fotos. Não precisa preencher valores.</p></div></div>
      <RequestSummary auth={autorizacao} posto={postoAtual} />
      <div className="rounded-lg bg-muted p-3 text-sm"><b>Ordem:</b> bomba → painel/KM → recibo do posto → finalizar.</div>
      <Button className="h-12 w-full" onClick={() => setStep("bomba")}><Camera className="mr-2 h-4 w-4" />Iniciar abastecimento</Button>
    </Card>}

    {step === "negado" && autorizacao && <Card className="space-y-4 border-red-500/30 p-5">
    ...
    {step === "bomba" && autorizacao && <Card className="space-y-4 p-4">
      <div><p className="text-xs font-semibold text-amber-500">ETAPA 1 DE 3</p><h2 className="text-lg font-bold">Foto da bomba</h2><p className="text-sm text-muted-foreground">Fotografe o visor completo com total, litros e preço por litro visíveis.</p></div>
    ...
    {step === "painel" && autorizacao && <Card className="space-y-4 p-4">
      <div><p className="text-xs font-semibold text-amber-500">ETAPA 2 DE 3</p><h2 className="text-lg font-bold">Foto do painel / KM</h2><p className="text-sm text-muted-foreground">Fotografe o painel com o hodômetro total visível.</p></div>
    ...
    {step === "recibo" && autorizacao && <Card className="space-y-4 p-4">
      <div><p className="text-xs font-semibold text-amber-500">ETAPA 3 DE 3</p><h2 className="text-lg font-bold">Foto do recibo do posto</h2><p className="text-sm text-muted-foreground">Fotografe o recibo inteiro, de cima até o valor total. Essa foto fica anexada ao comprovante.</p></div>
      <div className="grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-2 text-center text-emerald-600">Bomba salva</div><div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-2 text-center text-emerald-600">Painel salvo</div></div>
      {fotoReciboUrl && <img src={fotoReciboUrl} alt="Recibo do posto" className="max-h-80 w-full rounded-lg object-contain" />}
      <Button className="h-12 w-full" onClick={() => setCamRecibo(true)} disabled={loading || !fotoBombaUrl || !fotoPainelUrl}><ReceiptText className="mr-2 h-4 w-4" />{fotoReciboUrl ? "Trocar foto do recibo" : "Tirar foto do recibo"}</Button>
    </Card>}

    {step === "revisao" && autorizacao && <Card className="space-y-4 p-4">
      <div><p className="text-xs font-semibold text-emerald-500">CONFERÊNCIA FINAL</p><h2 className="text-lg font-bold">3 fotos prontas</h2><p className="mt-1 text-sm text-muted-foreground">Bomba + painel + recibo do posto. Ao finalizar, as 3 fotos e o GPS ficam no registro.</p></div>
      <RequestSummary auth={autorizacao} posto={postoAtual} />
      <div className="grid grid-cols-2 gap-2">
        <div><p className="mb-1 text-[11px] text-muted-foreground">Foto da bomba</p><img src={fotoBombaUrl} alt="Bomba" className="h-32 w-full rounded-lg object-cover" /></div>
        <div><p className="mb-1 text-[11px] text-muted-foreground">Foto do painel / KM</p><img src={fotoPainelUrl} alt="Painel" className="h-32 w-full rounded-lg object-cover" /></div>
      </div>
      <div><p className="mb-1 text-[11px] text-muted-foreground">Recibo do posto</p><img src={fotoReciboUrl} alt="Recibo do posto" className="max-h-72 w-full rounded-lg border object-contain" /></div>
      <div className="grid grid-cols-3 gap-2">
        <Button variant="outline" onClick={() => setCamBomba(true)} disabled={loading}><Camera className="h-4 w-4" /></Button>
        <Button variant="outline" onClick={() => setCamPainel(true)} disabled={loading}><Gauge className="h-4 w-4" /></Button>
    ...
    {step === "ok" && receipt && <Card className="space-y-4 border-emerald-500/30 p-5">
      <div className="flex items-center gap-3 text-emerald-500"><CheckCircle2 className="h-9 w-9" /><div><h2 className="text-lg font-bold">Abastecimento concluído</h2><p className="text-xs text-muted-foreground">Bomba, painel, recibo do posto e GPS foram salvos.</p></div></div>
      <RequestSummary auth={autorizacao!} posto={postoAtual} />
    ...
    <CameraCapture open={camRecibo} onClose={() => setCamRecibo(false)} onCapture={onCaptureRecibo} facing="environment" allowGallery title="Recibo do Posto" hint="Fotografe o recibo inteiro, incluindo posto, placa, litros e valor" />
  </div>;
}

function RequestSummary({ auth, posto }: { auth: Authorization; posto?: Posto | null }) {
  return <div className="grid grid-cols-2 gap-x-4 gap-y-2 rounded-lg bg-muted p-4 text-sm">
