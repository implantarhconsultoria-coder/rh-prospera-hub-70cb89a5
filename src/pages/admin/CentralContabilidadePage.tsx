...
import {
  AlertTriangle, BadgeCheck, Building2, CheckCircle2, Clock3, FileCheck2,
  FileText, Loader2, RefreshCw, Send, UploadCloud, Users,
} from 'lucide-react';
...
type Upload = {
  ...
  formalizacao_destinos?: string[] | null;
...
const statusLabel = (value?: string | null) => ({
  ...
  enviado: 'Formalizado',
  erro_envio_email: 'Falha no e-mail',
...
const CentralContabilidadePage: React.FC = () => {
  ...
  const carregar = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setRevisoes((r1.data || []) as any);
      setUploads((r2.data || []) as any);
      setPortalUsers((r3.data || []) as any);
    } catch (error: any) {
  ...
  const abrirUpload = async (upload: Upload) => {
    setBusyId(upload.id);
    try {
      ...
      const response = await fetch('/api/accounting-central-admin', {
        ...
        body: JSON.stringify({ action: 'view_upload', upload_id: upload.id }),
      });
      const data = await response.json();
      if (!response.ok || !data?.ok || !data?.url) throw new Error(data?.message || data?.error || 'Não foi possível abrir o documento.');
  ...
  const reenviarFormalizacao = async (upload: Upload) => {
    setBusyId(upload.id);
    try {
      const token = await getAuthToken();
      const response = await fetch('/api/accounting-central-admin', {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: 'retry_email', upload_id: upload.id }),
      });
      const data = await response.json();
      if (!response.ok || !data?.ok) throw new Error(data?.message || data?.error || 'Não foi possível reenviar a formalização.');
      if (data.email_status === 'enviado') toast.success('Formalização enviada por e-mail.');
      else toast.error(`O documento está na plataforma, mas o e-mail ainda não saiu: ${statusLabel(data.email_status)}.`);
      await carregar(true);
    } catch (error: any) {
      toast.error(error?.message || 'Erro ao reenviar formalização.');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const tabs: Array<{ key: TabKey; label: string; icon: React.ElementType }> = [
    ...
    { key: 'fechamento', label: 'Fechamento / Envio', icon: Send },
  ];
      ...
      <section className="overflow-hidden rounded-xl border border-[#2b2335] bg-[#05070b] shadow-[0_18px_50px_rgba(0,0,0,.24)]">
        <div className="flex flex-col gap-5 border-b border-[#28212f] bg-[radial-gradient(circle_at_10%_0%,rgba(139,34,255,.17),transparent_36%)] px-6 py-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black tracking-tight text-white">Central da Contabilidade</h1>
            <p className="mt-1 max-w-3xl text-sm text-zinc-400">Sua tela para acompanhar conferências da contabilidade, documentos recebidos, formalizações por e-mail e o fechamento que será enviado para processamento.</p>
          </div>
        ...
        <div className="flex gap-1 overflow-x-auto border-t border-[#211b28] bg-[#07090d] px-3 py-2">
          {tabs.map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => setTab(key)} className={`inline-flex shrink-0 items-center gap-2 rounded-md px-3 py-2 text-xs font-bold transition ${tab === key ? 'bg-[#25123d] text-white ring-1 ring-[#7131a8]' : 'text-zinc-500 hover:bg-white/[.035] hover:text-zinc-200'}`}>
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <Panel title="Últimas conferências" icon={FileCheck2}>
            {revisoes.length === 0 ? <Empty text="Nenhuma conferência registrada." /> : revisoes.slice(0, 8).map((r) => (
              <div key={r.id} className="flex items-start justify-between gap-3 border-b border-[#1f2026] px-1 py-3 last:border-0">
          ...
          <Panel title="Últimos documentos recebidos" icon={UploadCloud}>
            {uploads.length === 0 ? <Empty text="Nenhum documento recebido." /> : uploads.slice(0, 8).map((u) => (
              <div key={u.id} className="flex items-center justify-between gap-3 border-b border-[#1f2026] px-1 py-3 last:border-0">
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold text-zinc-100">{u.arquivo_nome}</div>
                  <div className="mt-1 text-xs text-zinc-500">{companyMap.get(u.empresa_id) || 'Empresa'} · {userMap.get(u.portal_user_id)?.nome || 'Contabilidade'} · {brDateTime(u.created_at)}</div>
                </div>
          ...
          {errosEmail.length > 0 && (
            <div className="xl:col-span-2">
              <Panel title="Formalizações de e-mail que precisam ser reenviadas" icon={AlertTriangle}>
                <div className="grid gap-2 lg:grid-cols-2">
                  {errosEmail.slice(0, 12).map((u) => (
                    <div key={u.id} className="rounded-lg border border-rose-500/20 bg-rose-500/[.045] p-3">
                      <div className="text-sm font-bold text-white">{u.arquivo_nome}</div>
                      <div className="mt-1 text-xs text-zinc-500">{companyMap.get(u.empresa_id) || 'Empresa'} · {brDateTime(u.created_at)}</div>
                      <div className="mt-3 flex items-center justify-between gap-2">
                        <StatusBadge value={u.formalizacao_email_status || 'erro_envio_email'} />
                        <button onClick={() => void reenviarFormalizacao(u)} disabled={busyId === u.id} className="inline-flex items-center gap-2 rounded-md bg-[#7c24d6] px-3 py-1.5 text-xs font-bold text-white hover:bg-[#9133ef] disabled:opacity-50">
                          {busyId === u.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />} Reenviar formalização
                        </button>
                      </div>
                    </div>
                  ))}
        ...
        <Panel title="Documentos enviados pela contabilidade para o RH" icon={UploadCloud}>
          {uploads.length === 0 ? <Empty text="Nenhum documento recebido." /> : (
            <div className="space-y-2">
              {uploads.map((u) => (
                <div key={u.id} className="grid gap-3 rounded-lg border border-[#24212a] bg-[#080a0e] p-4 lg:grid-cols-[1.5fr_.8fr_.8fr_.8fr_auto] lg:items-center">
                  <div className="min-w-0"><div className="truncate text-sm font-bold text-white">{u.arquivo_nome}</div><div className="mt-1 text-xs text-zinc-500">{tipoLabel(u.tipo_documento)}{u.funcionario_nome ? ` · ${u.funcionario_nome}` : ''}</div></div>
                  <div><div className="text-[10px] uppercase text-zinc-600">Empresa</div><div className="mt-1 text-xs font-semibold text-zinc-300">{companyMap.get(u.empresa_id) || '—'}</div></div>
                  <div><div className="text-[10px] uppercase text-zinc-600">Enviado por</div><div className="mt-1 text-xs font-semibold text-zinc-300">{userMap.get(u.portal_user_id)?.nome || 'Contabilidade'}</div></div>
                  <div><div className="text-[10px] uppercase text-zinc-600">Formalização</div><div className="mt-1"><StatusBadge value={u.formalizacao_email_status || '—'} /></div></div>
                  <div className="flex gap-2 lg:justify-end"><button onClick={() => void abrirUpload(u)} disabled={busyId === u.id} className="rounded-md border border-[#423051] px-3 py-2 text-xs font-bold text-zinc-200 hover:border-[#8b22ff]">Abrir PDF</button>{!emailOk(u.formalizacao_email_status) && <button onClick={() => void reenviarFormalizacao(u)} disabled={busyId === u.id} className="rounded-md bg-[#7c24d6] px-3 py-2 text-xs font-bold text-white hover:bg-[#9235ed]">Reenviar e-mail</button>}</div>
                </div>
              ))}
            </div>
          )}
...
const Panel = ({ title, icon: Icon, children }: { title: string; icon: React.ElementType; children: React.ReactNode }) => <section className="rounded-xl border border-[#27222e] bg-[#05070b] p-5 shadow-[0_12px_35px_rgba(0,0,0,.18)]"><div className="mb-4 flex items-center gap-2"><Icon className="h-4 w-4 text-[#ffc400]" /><h2 className="text-sm font-black uppercase tracking-wide text-white">{title}</h2></div>{children}</section>;

const Empty = ({ text }: { text: string }) => <div className="py-10 text-center text-sm text-zinc-600">{text}</div>;

const MetricCard = ({ icon: Icon, label, value, tone }: { icon: React.ElementType; label: string; value: number; tone: 'amber' | 'green' | 'purple' | 'blue' }) => {
  const tones = { amber: 'text-amber-300 border-amber-500/20 bg-amber-500/[.055]', green: 'text-emerald-300 border-emerald-500/20 bg-emerald-500/[.055]', purple: 'text-violet-300 border-violet-500/20 bg-violet-500/[.055]', blue: 'text-sky-300 border-sky-500/20 bg-sky-500/[.055]' };
  return <div className={`rounded-lg border p-4 ${tones[tone]}`}><div className="flex items-center justify-between"><div className="text-[10px] font-bold uppercase tracking-wider opacity-70">{label}</div><Icon className="h-4 w-4" /></div><div className="mt-2 text-2xl font-black text-white">{value}</div></div>;
};

const StatusBadge = ({ value }: { value?: string | null }) => {
  const v = String(value || '—');
  const cls = v === 'conferido' || v === 'enviado' ? 'border-emerald-500/25 bg-emerald-500/10 text-emerald-300' : v.includes('erro') || v === 'pendencia' ? 'border-rose-500/25 bg-rose-500/10 text-rose-300' : v === 'retificacao' ? 'border-violet-500/25 bg-violet-500/10 text-violet-300' : 'border-amber-500/25 bg-amber-500/10 text-amber-300';
  return <span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-bold ${cls}`}>{statusLabel(v)}</span>;
};
