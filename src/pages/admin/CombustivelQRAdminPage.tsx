...
type ReportMode = 'consolidado' | 'detalhado';
...
export default function CombustivelQRAdminPage() {
  ...
  const loadReport = useCallback(async () => {
    ...
    const { data, error } = await supabase.rpc('relatorio_abastecimento_periodo' as any, {
      p_data_inicio: period.startDate,
      p_data_fim: period.endDate,
    });
    setLoadingReport(false);
    if (error) {
      toast.error(error.message || 'Não foi possível consultar os abastecimentos.');
      return;
    }
    const rows = ((data as unknown[]) || []).map((item) => (typeof item === 'string' ? JSON.parse(item) : item)) as FuelReportRecord[];
    setRecords(rows);
    setLoadedPeriodLabel(period.label);
    setLoadedSuffix(`${period.startDate}_${period.endDate}`);
    toast.success(`${rows.length} abastecimento(s) localizado(s) no período.`);
  ...
  const openPdf = (mode: ReportMode, download = false) => {
    if (!filteredRecords.length) return toast.error('Não há dados no filtro atual.');
  ...
  const prepareEmail = (mode: ReportMode) => {
    ...
    const pdf = mode === 'consolidado'
      ? generateConsolidatedFuelPdf(consolidated, loadedPeriodLabel, loadedSuffix)
      : generateDetailedFuelPdf(filteredRecords, loadedPeriodLabel, loadedSuffix);
    const title = mode === 'consolidado' ? 'Relatório Consolidado de Abastecimentos' : 'Relatório Detalhado de Abastecimentos';
    setEmailDraft({
      to: [],
      cc: [...TOPAC_REPORT_CC],
      ...
      moduleOrigin: `relatorio-abastecimentos-${mode}`,
