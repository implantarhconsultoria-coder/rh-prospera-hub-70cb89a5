...
const GOIANIA_PLACAS = ["NFX8173", "QTP6H84", "OMV6480", "FYC6433", "FLN9450"];
...
export default function CombustivelQRAdminPage() {
  ...
  const filtrados = useMemo(() => abastecimentos.filter((a) => {
    ...
    if (fBusca) {
      ...
      const hay = `${a.mecanico_nome} ${a.placa || ""} ${a.posto_nome || ""} ${a.combustivel || ""} ${a.status || ""} ${a.empresa || ""}`.toLowerCase();
      if (!hay.includes(q)) return false;
    }
    return true;
  }), [abastecimentos, comp, fEmp, fPosto, fStatus, fBusca]);

  const inconsistencias = (a: Abast) => {
    const avisos: string[] = [];
    const empresa = `${a.empresa || ""} ${a.filial || ""}`.toUpperCase();
    const placaNorm = normalizePlate(a.placa);
    if (!a.foto_placa_url) avisos.push("Sem foto da placa");
    if (!a.foto_bomba_url) avisos.push("Sem foto da bomba");
    if (!a.foto_painel_url) avisos.push("Sem foto do KM");
    if (empresa.includes("GOI") && placaNorm && !GOIANIA_PLACAS.includes(placaNorm)) avisos.push("Veiculo fora de Goiania");
    const valor = Number(a.valor || 0);
    const litros = Number(a.litros || 0);
    const preco = litros > 0 ? valor / litros : 0;
    if (valor > 1500 || (preco > 0 && (preco < 1.5 || preco > 30))) avisos.push("Valor fora do padrao");
    return avisos;
  };
