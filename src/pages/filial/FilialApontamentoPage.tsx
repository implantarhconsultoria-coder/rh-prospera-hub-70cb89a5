...
const FilialApontamentoPage: React.FC = () => {
  ...
  const extEmpresa = normalize(`${ext.empresaNome} ${ext.filialNome}`);
  const filialAtual = ext.isExterno
    ? (ext.filialNome || (extEmpresa.includes('PRAIA') ? 'Praia Grande' : extEmpresa.includes('GOI') ? 'Goiania' : ''))
    : userRole === 'filial_praia' ? 'Praia Grande' : userRole === 'filial_goiania' ? 'Goiania' : '';
  const empresaNomeAtual = ext.isExterno
    ? (ext.empresaNome || filialAtual)
    : userRole === 'filial_praia' ? 'TOPAC FILIAL PRAIA GRANDE' : 'TOPAC FILIAL GOIANIA';
  ...
  useEffect(() => { if (filialAtual) fetchItems(); }, [filialAtual, competencia]);

  const funcionariosFilial = employees.filter(e => {
    const company = companies.find(c => c.id === e.companyId);
    const emp = normalize(company?.name || '');
    if (ext.isExterno) {
      if (ext.empresaIds && ext.empresaIds.length > 0) return ext.empresaIds.includes(e.companyId);
      if (extEmpresa.includes('PRAIA')) return emp.includes('PRAIA');
      if (extEmpresa.includes('GOI')) return emp.includes('GOI');
      return false;
    }
  ...
  const salvar = async (statusFinal: 'rascunho' | 'enviado') => {
    ...
    if (statusFinal === 'enviado') {
      await registrarAlertaFilial({
        filial: filialAtual, empresaNome: empresaNomeAtual,
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="card-premium p-6 gradient-primary text-primary-foreground">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold font-display">Apontamento — {filialAtual}</h1>
      ...
      <div className="card-premium p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <Input type="text" inputMode="decimal" value={quantidade} onChange={e => setQuantidade(e.target.value === '' ? '' : Number(e.target.value.replace(',', '.')))} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground block mb-1">Valor (R$)</label>
            <Input type="text" inputMode="decimal" value={valor} onChange={e => setValor(e.target.value === '' ? '' : Number(e.target.value.replace(',', '.')))} />
          </div>
