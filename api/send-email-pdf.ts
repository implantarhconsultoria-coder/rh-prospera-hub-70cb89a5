...
const buildMimeMessage = (payload: any, from: string) => {
  ...
  const attachmentParts = (payload.attachments || []).flatMap((attachment: any) => [
    `--${boundary}`,
    `Content-Type: ${attachment.attachmentContentType || PDF_CONTENT_TYPE}; name="${attachment.attachmentName}"`,
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      attachments: (payload.attachments || []).map((attachment: any) => ({
        ...
        content_type: attachment.attachmentContentType || PDF_CONTENT_TYPE,
      })),
    }),
  });
  ...
  const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
    ...
    body: JSON.stringify({
      ...
      attachments: (payload.attachments || []).map((attachment: any) => ({
        content: attachment.attachmentBase64,
        filename: attachment.attachmentName,
        type: attachment.attachmentContentType || PDF_CONTENT_TYPE,
...
export default async function handler(req: any, res?: any) {
  ...
  const payload = {
    ...
    attachmentSize: Number(firstAttachment.attachmentSize || 0),
    documentId: senderContext.documentId || firstAttachment.documentId || null,
    documentName: String(body.documentName || body.documentoNome || senderContext.documentName || firstAttachment.documentName || attachmentNames || '').trim(),
  };
