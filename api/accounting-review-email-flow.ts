...
const resolveMovement = async (service: any, originType: string, originId: string): Promise<Movement> => {
  if (originType === 'atestado_doc') {
    ...
    return {
      companyId: r.company_id, employeeName: r.funcionario_nome, reference: r.data_documento,
      ...
      bucket: r.storage_bucket || 'documentos-funcionarios', path: r.storage_path || r.arquivo_url,
    };
  }
  if (originType === 'atestado') {
    ...
    if (error || !r) throw Object.assign(new Error('movimento_nao_encontrado'), { status: 404 });
    return {
      companyId: r.company_id, employeeName: r.funcionario_nome, reference: r.competencia || r.data_inicio,
      label: 'Atestado / afastamento', subjectLabel: 'Atestado conferido',
      confirmationText: 'O atestado/afastamento foi conferido pela Contabilidade e está liberado para continuidade do RH.',
      fileName: r.arquivo_nome || fileNameFromPath(r.arquivo_url), bucket: 'atestados', path: r.arquivo_url,
    };
  }
  if (originType === 'admissao') {
    ...
    return {
      companyId: r.empresa_id, employeeName: r.nome, reference: r.data_admissao,
      label: 'Admissão', subjectLabel: 'Admissão conferida',
      confirmationText: 'A documentação admissional foi conferida pela Contabilidade e está liberada para continuidade do RH.',
      fileName: path ? fileNameFromPath(path) : null, bucket: path ? 'documentos-admissionais' : null, path: path || null,
    };
  }
  if (originType === 'demissao') {
    ...
    if (error || !r) throw Object.assign(new Error('movimento_nao_encontrado'), { status: 404 });
    return {
      companyId: r.company_id, employeeName: r.funcionario_nome, reference: r.data_desligamento,
      label: 'Demissão / rescisão', subjectLabel: 'Demissão / rescisão conferida',
  ...
  if (originType === 'ferias') {
    ...
    return {
      companyId: r.company_id, employeeName: r.funcionario_nome, reference: r.periodo_gozo_inicio,
      label: 'Férias', subjectLabel: 'Férias conferidas',
      confirmationText: 'A movimentação de férias foi conferida pela Contabilidade e está liberada para continuidade do RH.',
      fileName: path ? fileNameFromPath(path) : null, bucket: path ? 'documentos-funcionarios' : null, path: path || null,
    };
  }
  if (originType === 'fechamento') {
    ...
    if (error || !r) throw Object.assign(new Error('movimento_nao_encontrado'), { status: 404 });
    return {
      companyId: r.company_id, companyName: r.empresa_nome, reference: r.competencia,
      label: 'Fechamento da folha', subjectLabel: 'Fechamento conferido',
  ...
  if (originType === 'alerta') {
    ...
    return {
      companyId: r.empresa_id, employeeName: r.funcionario_nome, reference: r.detalhes?.competencia || r.detalhes?.data_registro || null,
      label, subjectLabel: `${label} conferida`,
...
const routingFor = async (service: any, portal: string, senderEmail: string) => {
  ...
  return { to: [primary], cc: configured.slice(1).filter((e) => e !== sender && e !== primary) };
};
...
const logEmail = async (service: any, input: any) => {
  try {
    await service.from('email_envios_log').insert(input);
  } catch (error) {
...
export default async function handler(req: any, res?: any) {
  ...
  try {
    ...
    const cc = validEmails(input.cc);
    const subject = clean(input.subject) || prepared.subject;
    const body = clean(input.body) || prepared.body;
    if (!to.length || !subject || !body) return sendJson(res, { ok: false, error: 'dados_invalidos', message: 'Destinatário, assunto e mensagem são obrigatórios.' }, 400);
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from, to, ...(cc.length ? { cc } : {}), reply_to: replyTo,
    ...
    if (!response.ok) {
      await logEmail(service, {
        user_id: null, usuario_nome: prepared.user.nome, email_corporativo_usado: prepared.user.email || null,
        email_remetente: from, reply_to: replyTo, provider: 'resend', modulo_origem: 'contabilidade_confirmacao',
        documento_id: null, documento_nome: prepared.movement.fileName || prepared.movement.label,
        destinatarios: to.join(', '), cc: cc.join(', '), assunto: subject, status: 'erro', erro: detail.slice(0, 1000), enviado_em: new Date().toISOString(),
      });
    ...
    await logEmail(service, {
      ...
      email_remetente: from, reply_to: replyTo, provider: 'resend', modulo_origem: 'contabilidade_confirmacao',
      documento_id: null, documento_nome: prepared.movement.fileName || prepared.movement.label,
      destinatarios: to.join(', '), cc: cc.join(', '), assunto: subject, status: 'enviado', erro: null, enviado_em: new Date().toISOString(),
    });
    ...
    return sendJson(res, { ok: false, error: clean(error?.message || error), detail: clean(error?.detail).slice(0, 500) }, Number(error?.status || 500));
  }
}
