...
const sendFormalizationEmail = async (service: any, input: {
  ...
  const { data: config } = await service
    ...
    .maybeSingle();
  const recipients = config?.ativo ? cleanEmails(config?.formalizacao_destinos) : [];
  if (!recipients.length) return { status: 'aguardando_configuracao', recipients: [] as string[] };

  const resendKey = String(process.env.RESEND_API_KEY || '').trim();
  if (!resendKey) return { status: 'erro_configuracao_email', recipients };
  ...
  const text = [
    ...
    `Enviado por: ${input.userName}`,
    `Data e hora do registro: ${new Date(input.createdAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`,
    input.observation ? `Observação: ${input.observation}` : '',
    `ID da operação: ${input.uploadId}`,
    '',
    'O arquivo foi recebido e armazenado diretamente na plataforma. Este e-mail formaliza o registro da operação.',
    '',
    'TOPAC RH PRO',
  ].filter(Boolean).join('\n');
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#111827;line-height:1.55;max-width:680px">
      ...
      <table style="width:100%;border-collapse:collapse;margin:18px 0">
        ...
        <tr><td style="padding:8px;border-bottom:1px solid #e5e7eb"><b>Enviado por</b></td><td style="padding:8px;border-bottom:1px solid #e5e7eb">${htmlEscape(input.userName)}</td></tr>
        <tr><td style="padding:8px;border-bottom:1px solid #e5e7eb"><b>Registrado em</b></td><td style="padding:8px;border-bottom:1px solid #e5e7eb">${htmlEscape(new Date(input.createdAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }))}</td></tr>
      ...
      <p><b>O arquivo já foi recebido e armazenado na plataforma.</b> Este e-mail serve exclusivamente para formalizar o registro da operação.</p>
    </div>`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { status: 'erro_envio_email', recipients };
    }
    return { status: 'enviado', recipients };
  } catch (error) {
...
export default async function handler(req: any, res?: any) {
  ...
  try {
    ...
    if (action === 'prepare') {
      ...
      if (!Number.isFinite(fileSize) || fileSize <= 0 || fileSize > MAX_FILE_BYTES) return sendJson(res, { ok: false, error: 'tamanho_invalido', max_bytes: MAX_FILE_BYTES }, 400);
    ...
    if (action === 'finalize') {
      ...
      const { data: existing } = await service.from('contabilidade_portal_uploads').select('id,formalizacao_email_status,formalizacao_email_em').eq('storage_path', storagePath).maybeSingle();
      if (existing) return sendJson(res, { ok: true, upload_id: existing.id, duplicate_finalize: true, email_status: existing.formalizacao_email_status, formalizado_em: existing.formalizacao_email_em });
      ...
      await service.from('contabilidade_portal_uploads').update({
        ...
        formalizacao_destinos: mail.recipients,
        updated_at: new Date().toISOString(),
      }).eq('id', upload.id);

      return sendJson(res, { ok: true, upload_id: upload.id, email_status: mail.status, formalizado_em: formalizedAt });
    }

    if (action === 'view') {
      const uploadId = String(body.upload_id || '').trim();
      const user = await validateSession(service, portal, token);
      const { data: upload, error } = await service.from('contabilidade_portal_uploads').select('id,portal_user_id,empresa_id,storage_bucket,storage_path,arquivo_nome').eq('id', uploadId).maybeSingle();
      if (error || !upload) return sendJson(res, { ok: false, error: 'documento_nao_encontrado' }, 404);
      const { data: allowed } = await service.from('contabilidade_portal_acesso_empresas').select('empresa_id').eq('portal_user_id', user.id).eq('empresa_id', upload.empresa_id).maybeSingle();
